...
# <<< REVISION 1 (2025-04-09) - Applied based on user comments >>> # Updated Date
# --- CHANGES ---
...
def unlock_funds(
    ...
    try:
        amount_decimal = Decimal(str(amount)) if not isinstance(amount, Decimal) else amount
        if amount_decimal <= Decimal('0.0'):
            # Changed to log info and return None instead of raising error for non-positive unlock
    ...
    try:
        ...
        return None # Nothing to unlock if balance doesn't exist
    except Exception as e:
