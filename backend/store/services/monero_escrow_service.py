...
#   - (This addresses pytest failures related to AttributeError for these functions).
# - Corrected Part 1: (Original version provided by user)

# Original Revision Notes relevant to XMR Escrow Process:
# (Include relevant notes from escrow_service.py if any were specifically about XMR issues/fixes)
# - v1.22.1: Pass order ID as keyword argument 'order_guid' to monero_service.create_monero_multisig_wallet
#           to match test assertion expectation.
# - v1.19.0: Passing full `order` object to sign_xmr_multisig_tx slightly breaks abstraction.
#           Consider passing only needed data in future refactor.
...
# === Enterprise Grade Monero Escrow Functions ===

@transaction.atomic
# RENAMED: v1.23.0 - Aligned with common_escrow_utils dispatcher expectation
def create_escrow(order: 'Order') -> None:
    ...
    if not all([CryptoPayment, User, GlobalSettings, monero_service]):
    ...
    try:
        # CORRECTED: Use timedelta (now imported)
...
def check_and_confirm_payment(payment_id: Any) -> None:
    ...
    Ledger (using standard XMR units) and Order status.
    ...
    try:
        ...
        # CORRECTED: Added except clauses
        try:
...
# --- End of Part 1 of Updated File ---
# <<< Part 2: Continues from monero_escrow_service.py (Updated Part 1) >>>
# REVISIONS:
# - 2025-04-09 (The Void): v1.23.0 - Renamed create_escrow, broadcast_release.
# - (No changes in this section for v1.23.0)
...
def mark_order_shipped(order: 'Order', vendor: 'UserModel', tracking_info: Optional[str] = None) -> None:
    ...
    if not all([Order, GlobalSettings, User, monero_service]):
        raise RuntimeError("Critical application models or monero_service are not available.")
    if order.selected_currency != CURRENCY_CODE:
        raise ValueError(f"This service only handles {CURRENCY_CODE} orders.")
    ...
    try:
        ...
        # Use the XMR-specific internal helper
        prepared_release_metadata = _prepare_xmr_release(order_locked)
    ...
    # --- Save Order Updates ---
    try:
        # Note: XMR path doesn't have the same known validation issue as BTC path had with address formats.
        # Keep full_clean for general data integrity.
...
def sign_order_release(order: 'Order', user: 'UserModel', private_key_info: str) -> Tuple[bool, bool]:
    ...
    Args:
        ...
        private_key_info: User's private key or signing credential info (format specific to monero_service).
    ...
    # --- Input and Dependency Validation ---
    if not all([Order, User, monero_service]):
        raise RuntimeError("Critical application models or monero_service are not available.")
    ...
    if not private_key_info or not isinstance(private_key_info, str) or len(private_key_info) < 10:
    ...
    try:
        ...
        # Note: Passing full `order_locked` object might be necessary if monero_service needs wallet name/info from it.
        # Consider refactoring monero_service to accept only necessary data if possible.
        ...
        # Update the current signatures map with the map returned by the crypto service
        # For XMR, the service might return a complete map, so replace rather than update.
        # Verify this assumption based on monero_service implementation. Assuming replacement here:
        current_sigs = updated_sigs_map

        # Recalculate completeness based on the final map (defense-in-depth)
...
# --- End of Part 2 of Updated File ---
# <<< Part 3: Continues from monero_escrow_service.py Updated Part 2 >>>
# REVISIONS:
# - 2025-04-09 (The Void): v1.23.0 - Renamed functions to align with common_escrow_utils dispatcher.
#   - Renamed `create_escrow_for_order` to `create_escrow`.
#   - Renamed `broadcast_release_transaction` to `broadcast_release`.
#   - (This addresses pytest failures related to AttributeError for these functions).
# - (No other changes in this section for v1.23.0)
...
def broadcast_release(order_id: Any) -> bool:
    ...
    # --- Dependency Check ---
    if not all([ledger_service, Order, User, monero_service]):
        raise RuntimeError("Critical application components (Ledger, Models, monero_service) are not available.")
...
# --- Internal Helper: _prepare_xmr_release ---
def _prepare_xmr_release(order: 'Order') -> Dict[str, Any]:
    """
...
def resolve_dispute(
    ...
    # --- Dependency Checks ---
    if not all([ledger_service, Order, User, GlobalSettings, monero_service]):
    ...
    try:
        ...
        if release_to_buyer_percent > 0:
            buyer_share_xmr = (total_escrowed_xmr * Decimal(release_to_buyer_percent) / Decimal(100)).quantize(quantizer, rounding=ROUND_DOWN)
