...
"""
# <<< ENTERPRISE GRADE REVISION: v1.5.2 - Correct Critical Field Deletion and Type Error >>>
# Revision Notes:
# - v1.5.2 (2025-06-28):
#   - CRITICAL FIX: Re-added the `selected_currency` field to the `Order` model, which was
#     accidentally removed in the previous revision. Its absence caused a `FieldDoesNotExist`
#     error at application startup.
#   - FIXED: Modified the `Order.calculate_total_price_native` method to robustly handle
#     numeric string inputs by explicitly casting them to `Decimal`. This resolves the
#     `TypeError` that occurred during `Order.objects.create` in test setups.
# - v1.5.0 (2025-06-07):
#   - FIXED: Moved the `SupportTicketStatus` enum to be a nested `StatusChoices` class inside the `SupportTicket` model. This aligns its structure with the `Order` and `Dispute` models for consistency.
#   - FIXED: Updated the `status` field on the `SupportTicket` model to reference the new nested `StatusChoices` class.
#   - Rationale: This resolves the `AttributeError: type object 'SupportTicket' has no attribute 'StatusChoices'` that was occurring in `tickets.py`, which expected the nested structure. This change promotes a consistent design pattern across models.
# - v1.4.3 (2025-05-22):
#   - Order.save():
...
class AuditLogAction(models.TextChoices):
    ...
    VENDOR_APP_APPROVE = 'vendor_app_approve', 'Vendor Application Approved'
    VENDOR_APP_REJECT = 'vendor_app_reject', 'Vendor Application Rejected'
    ORDER_UPDATE_STATUS = 'order_update_status', 'Order Status Updated'
    DISPUTE_OPEN = 'dispute_open', 'Dispute Opened'
...
class GlobalSettings(SingletonModel):
    ...
    allow_new_vendors = models.BooleanField(default=True, help_text="Controls if users can apply to become vendors (via the new application process).")
...
class UserManager(BaseUserManager):
    ...
    def _validate_pgp(self, pgp_public_key: Optional[str]):
        ...
        """
        if pgp_public_key and pgp_public_key.strip():
            try:
...
class User(AbstractBaseUser, PermissionsMixin):
    # (Content of User model is unchanged)
    # ...
    ...
    vendor_bond_paid = models.BooleanField(default=False, help_text="Indicates if the vendor bond has been paid and accepted (LEGACY - check VendorApplication status).")
    ...
    def clean(self):
        ...
        if self.xmr_multisig_info:
            if not isinstance(self.xmr_multisig_info, str) or len(self.xmr_multisig_info) < 100:
...
class Category(models.Model):
    # (Content of Category model is unchanged)
    # ...
    id = models.BigAutoField(primary_key=True)
    name = models.CharField(max_length=100, unique=True)
    slug = models.SlugField(max_length=110, unique=True, db_index=True, help_text="URL-friendly name. Usually set automatically from name.")
...
class Product(models.Model):
    # (Content of Product model is unchanged)
    # ...
...
class CryptoPayment(models.Model):
    # (Content of CryptoPayment model is unchanged)
    # ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
class Order(models.Model):
    ...
    release_tx_broadcast_hash = models.CharField(max_length=256, blank=True, null=True, db_index=True, help_text="Transaction Hash of the final broadcasted release transaction (longer for some chains).")
    ...
    def calculate_total_price_native(self) -> Decimal:
        ...
        Robustly handles both numeric and string-based numeric inputs.
        """
        try:
            # Explicitly cast to Decimal to prevent TypeError with string inputs
            product_price = Decimal(self.price_native_selected) if self.price_native_selected is not None else Decimal(0)
            shipping_price = Decimal(self.shipping_price_native_selected) if self.shipping_price_native_selected is not None else Decimal(0)
            # Quantity should be an int, but casting to Decimal is safe.
            order_quantity = Decimal(self.quantity) if self.quantity is not None else Decimal(0)
        except (ValueError, TypeError, InvalidOperation) as e:
            logger.error(
                f"Order {self.id or 'NEW'} failed to calculate total price due to invalid non-numeric input. Error: {e}. "
                f"Inputs: price='{self.price_native_selected}', shipping='{self.shipping_price_native_selected}', qty='{self.quantity}'"
            )
            return Decimal(0)
    ...
    def save(self, *args, **kwargs):
        ...
        elif self.total_price_native_selected is None:
...
class Feedback(models.Model):
    # (Content of Feedback model is unchanged)
    # ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
class Dispute(models.Model):
    # (Content of Dispute model is unchanged)
    # ...
    class StatusChoices(models.TextChoices):
        OPEN = 'open', _('Open')
...
class SupportTicket(models.Model):
    # (Content of SupportTicket model is unchanged)
    # ...
    class StatusChoices(models.TextChoices):
        OPEN = 'open', 'Open'
...
class TicketMessage(models.Model):
    # (Content of TicketMessage model is unchanged)
    # ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
class AuditLog(models.Model):
    # (Content of AuditLog model is unchanged)
    # ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
class WebAuthnCredential(models.Model):
    # (Content of WebAuthnCredential model is unchanged)
    # ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
...
class VendorApplication(models.Model):
    # (Content of VendorApplication model is unchanged)
    # ...
    class StatusChoices(models.TextChoices):
