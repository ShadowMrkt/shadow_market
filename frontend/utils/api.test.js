...
// --- REVISION HISTORY ---
// 2025-04-23: Rev 7 - [Gemini] Added jest.spyOn for console.warn/error in specific error-handling tests to suppress expected logs during test runs.
// 2025-04-16: Rev 6 - [Gemini] Added explicit `message` check to `toMatchObject` for 400 JSON error test.
...
describe('API Utility Functions', () => {
 ...
 describe('apiRequest Error Handling', () => {
    ...
    test('throws error with status for server errors (e.g., 500)', async () => {
      // <<< REVISION 7: Suppress expected console.warn >>>
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Received error status 500 with non-JSON content-type: text/html'));
      consoleWarnSpy.mockRestore(); // <<< REVISION 7: Restore spy >>>
    });

    // REFACTORED: Removed try/catch, uses .rejects.toMatchObject()
    test('throws specific "Unauthorized" error for 401 status', async () => {
      // <<< REVISION 7: Suppress expected console.error >>>
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Unauthorized access detected:', '/api/store/users/me/', errorBody);
      consoleErrorSpy.mockRestore(); // <<< REVISION 7: Restore spy >>>
    });
    ...
    test('throws specific "Forbidden" error for 403 status', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Forbidden access detected:', '/api/store/users/me/', errorBody);
      consoleErrorSpy.mockRestore(); // <<< REVISION 7: Restore spy >>>
    });

    // REFACTORED: Removed try/catch, uses .rejects.toMatchObject()
    // --- FIXED: Added explicit message check ---
    test('parses and includes JSON error details in thrown error message (e.g., 400)', async () => {
      ...
      await expect(loginInit({})).rejects.toMatchObject({
          ...
          status: 400,
          data: errorBody,
          // Explicitly check the message format expected based on previous failure diff
          message: `API Error (400): ${JSON.stringify(errorBody)}`
      });
      // No console log expected here in the current api.js implementation for 400
    });
    ...
    test('handles network errors during fetch', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Network Error during fetch for /api/store/users/me/:', networkError);
      consoleErrorSpy.mockRestore(); // <<< REVISION 7: Restore spy >>>
    });
 });
